...
RCT_EXPORT_METHOD(castMedia: (NSDictionary *)params
                  resolver:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    int mediaType = [RCTConvert int:params[@"mediaType"]];
    ...
    double sectionDuration = [RCTConvert double:params[@"sectionDuration"]];
    double sectionStartAbsoluteTime = [RCTConvert double:params[@"sectionStartAbsoluteTime"]];
    
    playPosition = !playPosition ? 0 : playPosition;
    
    GCKMediaMetadata *metadata =
    [[GCKMediaMetadata alloc] initWithMetadataType:mediaType];
